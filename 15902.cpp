...
#include <vector>
using namespace std;
int l, n, m, tmp, min_path = 0;
int board[3001];
int result[3001];
long long dp[3001] = {0};
long long comb[3001][3001] = {0};
long long factorial[3001] = {1};
...
long long get_comb(int n, int r)
{
   ...
   ret = get_comb(n-1, r) % MOD + get_comb(n-1, r-1) % MOD;
   ret %= MOD;
   return ret;
}
...
long long get_dp(int n)
{
   ...
   for(int i = 0; i < n; i+=2)
   {
      ret += (((get_dp(i) * get_dp(n-1-i)) % MOD) * get_comb(n-1, i)) % MOD;
      ret %= MOD;
   }
   return ret;
}
...
int main()
{
   ...
   int bitString = 0;
   long long res = 1, res_up = 0, res_down = 1;
	long long lla=2, llb=33;
	cout<<lla<<"^"<<llb<<"mod"<<MOD<<"="<<power(lla, llb)<<endl;
   cin >> l >> n;
   ...
   for(int i = 0; i < l-1; i++)
   {
      if(result[i] == board[i])
      {
         if(result[i] == 1)
         {
            ...
            for(int j = i+1; j < l; j++)
            {
               if(result[j] == board[j] && result[j] == 1) tmp++;
               else break;
            }
            if(bitString > 0) bitStrings.push_back(bitString);
            bitString = 0;
            i += tmp;
         }
         continue;
   ...
   {
      res *= get_dp(bitStrings[i]);
      res %= MOD;
      res_up += bitStrings[i];
      res_down *= get_fact(bitStrings[i]);
      res_down %= MOD;
   }
   res *= get_fact(res_up) * power(res_down, MOD-2) % MOD;
   res %= MOD;
   cout << min_path << " " << res << endl;
}